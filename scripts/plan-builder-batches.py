...
def build_plan(repository_root, lock_name, status_name, closure_name):
    ...
    # The plan cites the campaign that sets its round bound. Nothing downstream
    # reads a JSON string, so a citation that stopped resolving would travel in
    # every plan as an authority no reader can reach.
    repository_file(repository_root, BATCH_BOUND_EVIDENCE, "batch bound evidence")
    ...
    plan = {
        ...
        "stock_kernel_batch_bound": {
            ...
            "evidence": repository_name(repository_root, BATCH_BOUND_EVIDENCE,
                                        "batch bound evidence"),
            "reason": (
