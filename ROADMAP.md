...
52a. [x] Define machine-readable evidence classes and retain raw evidence. `scripts/capture-runtime-evidence.py` labels every field `observed`, `derived`, `declared`, or `not-captured`, and each probe retains its argv, exit status, both output streams, and the SHA-256 of stdout. `docs/audits/runtime-evidence-capture-protocol.md` carries the contract and `schemas/runtime-evidence-v2.schema.json` the schema
52b. [x] Bind declared configuration, container state, QEMU process, mounts, image metadata, monitor, and guest probe to one explicitly identified VM instance. The container is selected by finding a `qemu-system-x86_64` process rather than by name, two running instances exit 2 rather than resolving arbitrarily, the Compose service comes from the `com.docker.compose.service` label, declared configuration is read from that service alone, the command line comes from `/proc/<pid>/cmdline`, and the image is resolved through the container's own mount table. `declared.environment` and `live_container.environment` are recorded separately because a container may predate current source
52c. [ ] Validate the schema in CI and cover it with negative fixtures: duplicate environment keys across services, several QEMU containers, named-volume image storage, a custom monitor port, a custom published SSH port, an empty snapshot set, an SSH banner timeout, a missing guest key, a secret-bearing resolved configuration, a failed enumerator, and a dirty worktree. The committed cases are real sanitized captures and serve as examples, so schema regressions are not yet covered by a test
52d. [x] Produce a matched TCG/KVM contrast pair from one host, source commit, container image, guest image, machine, controller, and thread count, differing only in `FORCE_KVM`. `evidence/runtime/cases/accelerator-kvm-forced` and `evidence/runtime/cases/accelerator-tcg-ide-demoted` record `/dev/kvm` usable inside the container in both arms, which removes container device access as an explanation and establishes the `AUTO_DISABLE_KVM_FOR_IDE` demotion as an observation
