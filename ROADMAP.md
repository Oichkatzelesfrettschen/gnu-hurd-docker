...
55. [x] Report the MATE package closure for both Hurd ports from the archive rather than the guest. `Dockerfile.hurd-archive` sets `APT::Architecture` to `hurd-amd64` or `hurd-i386` over a private apt state tree, and `make hurd-closure` classifies a set as native, architecture-all, uninstallable, or missing. LMDE 7 `gigi` publishes no `binary-all` directory, so the overlay keeps only its `Architecture: all` stanzas and republishes them under the target architecture with Debian Ports pinned authoritative. Availability and dependency facts are archive facts; runtime behavior still needs the guest
55a. [ ] Build `mate-settings-daemon` and `mate-control-center` for `hurd-amd64`. Neither has a 64-bit binary while both exist for `hurd-i386`, and the main archive carries `mate-settings-daemon` source at 1.26.1-1.2, the version the i386 binary was built from, so this is a build gap rather than a porting gap. `scripts/build-hurd-package-in-guest.sh` rebuilds on the guest behind a required snapshot
55b. [ ] Answer `polkitd`, which is absent for both ports and is the single root blocker of every uninstallable MATE package on `hurd-i386`. `accountsservice`, `systemd`, `libelogind0`, and `policykit-1` are absent with it, so the session and seat layer is the boundary rather than Caja or Marco. `scripts/make-hurd-dependency-stub.sh` builds a recorded equivs stub for a single-user VM; a real port stays the outstanding work
55c. [ ] Rebuild `python3-setproctitle` for the Hurd, the one dependency keeping `mintmenu` out of the 37 Mint `Architecture: all` packages that already resolve
