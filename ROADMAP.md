...
52. [x] Make the runtime self-identifying, so operators read the accelerator QEMU selected rather than infer it from a target name. `scripts/capture-runtime-evidence.sh` records the container runtime, QEMU binary, accelerator (from the argv and the monitor), machine, disk bus, image path, and image digest, and labels each field `observed`, `derived`, `declared`, or `not-captured` so a request is never read as an outcome. `make runtime-info` prints it through `scripts/report-runtime-evidence.py`, and the `up-kvm` help text says that the overlay exposes `/dev/kvm` while the entrypoint selects the accelerator. The committed exemplar records the case the item was written against: `/dev/kvm` present, `FORCE_KVM=0`, and QEMU running `tcg,thread=multi`. `docs/audits/runtime-evidence-capture-protocol.md` carries the schema and the evidence-class contract. The image digest stays opt-in behind `--image-digest`, because hashing a 16 GiB qcow2 costs minutes
