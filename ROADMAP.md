...
52a2. [x] Retain both sanitized streams and certify them after redaction. Redaction runs inside `Capture.run` before each stream is written and before its digest is taken, and both `stdout_sha256` and `stderr_sha256` are recorded. The committed cases were sanitized after hashing, so four stdout digests per arm -- `host-uname`, `container-inspect`, `compose-config`, `image-info`, exactly the streams carrying the host name or a local path -- described bytes that no longer existed. `scripts/check-runtime-evidence.py` recomputes every digest, so that failure is now caught rather than published
52b. [ ] Bind to exactly one QEMU process rather than one container. Container selection by live QEMU process, the Compose service from the `com.docker.compose.service` label, declared configuration read from that service alone, argv from `/proc/<pid>/cmdline`, and image resolution through the container's mount table are in place, and several PIDs in one container now exit 2 rather than binding an arbitrary one. Offline image identity closes when a recapture exercises `--image PATH` against a stopped guest
52c. [x] Validate the schema and the evidence package in CI, covered by negative fixtures. `make evidence-check` runs `tests/runtime-evidence/test-runtime-evidence-contract.py` and validates every tracked capture, and Static Gates runs it. The fixtures assert what the contract rejects: a null value classed `observed`, a null value without a reason, a stdout or stderr digest that does not describe its file, a probe advertising an absent stream, a probe recording no stderr digest, a home path or a scratchpad path in a capture published as redacted, a wrong schema version, a missing section, and a directory with no `capture.json`. Remaining runtime fixtures -- several QEMU containers, named-volume image storage, a custom monitor port, an SSH banner timeout, a failed enumerator -- need a live runtime and land with the recapture
