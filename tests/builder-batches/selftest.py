...
def main():
    ...
    # The plan records the campaign that sets its round bound as a provenance
    # string, and the release archive withholds docs/reports/, so the planner
    # cannot open it without failing inside the product. The repository is where
    # the citation has to resolve, and this is what refuses a dangling one.
    with open(PLANNER, encoding="utf-8") as handle:
        cited = re.search(r'^BATCH_BOUND_EVIDENCE = "([^"]+)"', handle.read(),
                          re.MULTILINE)
    suite.check("the planner declares a round-bound citation", cited is not None)
    suite.check("the cited round-bound campaign resolves in the repository",
                cited is not None
                and os.path.isfile(os.path.join(ROOT, cited.group(1))),
                cited.group(1) if cited else "")
